...
| Sign-out everywhere | Redis blacklist of access tokens (§5.1) | A password reset and a "this was not me" revocation write the instant to Redis (`auth:nbf:<user id>`, kept for the access-token lifetime); the resource server refuses any access token whose `iat` is not later than that second, so already issued tokens stop working at once instead of within 15 minutes; the key is written after the revoking transaction commits and lives for the token lifetime plus the 60-second `exp` leeway. Without Redis the check is skipped and logged. Tokens are otherwise stateless: no per-token blacklist | One `GET` per API call is cheap and only the user's own tokens are affected; a blacklist of individual tokens would not cover tokens the server cannot enumerate |
| Password policy | §2 (length, breach list) | 10–72 bytes, not in the common-password list, and the new password must differ from the current one at reset (`password-same-as-current`); no password history is stored. A refused reset leaves the link usable, so a link holder could test candidate passwords against the current one within the request limit; accepted, the link is single-use, short-lived and only ever mailed to the account owner | Reusing the just-reset password defeats the reset; a history table keeps old hashes for little gain |
| Login page detours | Not specified | `/` and `/login` opened by a signed-in browser redirect to the application; a sign-in without a pending authorization request lands on the application; a login form posted with a stale CSRF token (server restart, session expiry) returns to the form with "the page has expired"; 403 and server errors render a branded page with a link to the application | The authorization server is reachable directly and should never dead-end in a 404 or a default error page |
| Organisation scope | ABAC policy engine with department/faculty attributes (§4) | Every role assignment is scoped to an organisation; the access token carries `role_scopes` (`ROLE:orgId`) and the resource server keeps the organisation tree in memory (reloaded on a timer and after changes). `@PreAuthorize` expressions ask `@access.inScope(orgId)`, `@access.below(role)` and `@access.canManage(role, orgId)`; the line of authority is `EMPLOYEE < FACULTY_SECRETARY < DEAN < RECTOR_SECRETARY < RECTOR`, `SYSTEM_ADMIN` is above everything and alone grants system roles, `GDPR_OFFICER` grants nothing. `user:read:scope` and `user:manage:scope` complete the matrix for the four approval roles; the directory is limited to the scopes of roles that grant `user:read:scope`, so an `EMPLOYEE` role elsewhere does not widen it. Calendar days are Kyiv days (application clock) | No consumer needs risk or time-of-day conditions before Epic 4; one map lookup per check and a small token beat a policy engine and per-request queries |
| Role assignment | Not specified | `POST /api/v1/users/{id}/roles` and `DELETE /api/v1/users/{id}/roles/{roleId}` need `user:manage` (anything, anywhere) or `user:manage:scope` (a role strictly below one the caller holds, in an organisation inside that role's subtree); the organisation must fit the role's level of the tree. Revocation never deletes: `valid_to` becomes yesterday and the former holder is signed out everywhere, so lost authority stops applying at once, while a grant reaches the token with the next silent refresh. Nobody may take back their own last role above `EMPLOYEE`. Both write `ROLE_ASSIGNED`/`ROLE_REVOKED` to `audit_logs` (entity type `AUTHORIZATION`, in the same transaction as the change) and email the person whose authority changed | The rule reconciles US-002, the RBAC matrix and §3.3 without an approval workflow; keeping the history makes "who could approve this in March" answerable in Epic 4 |
| Membership confirmation | Not specified | Self-registration grants no role: an `@chnu.edu.ua` address proves employment at the university, not membership of the department the person picked, so the first role is granted by somebody who may manage that department. `GET /users/me` reports `membershipConfirmed`; until it is true the token carries no roles or permissions and the app shows a banner. The answer comes from a `MembershipConfirmation` strategy whose only implementation defers to a person — the seam for a future LDAP or HR lookup. A `PENDING` account whose newest verification link has expired is replaced by the next registration for that address | Without it anybody with a university address could act as a member of any department |
| Approval delegation | Not specified | An approval role (`FACULTY_SECRETARY`, `DEAN`, `RECTOR_SECRETARY`, `RECTOR`) is lent to a colleague for at most 90 days through `POST /api/v1/delegations`; `role_delegations` keeps it out of the role history. The delegate's access token gains `delegations` (`ROLE:orgId:delegatorId`), the matching `role_scopes` entry and the role's `award:read:*` and `award:approve:*` permissions only. Because a borrowed role must not confer authority over people, the token also carries `held_scopes` — the scopes of the roles the person holds in their own right — and `@access.canManage`, `@access.below` and the directory scope read those, while `@access.inScope` accepts borrowed ones; a delegate can therefore approve inside the organisation but neither manage its users nor lend the role on, and receiving a delegation of a role one already holds does not take the held one away. A delegation is only read into a token while its delegator still holds the lent role, so it never outlives the authority it came from. The delegation expires by calendar date without any job; `DELETE /api/v1/delegations/{id}` by the delegator, `user:manage` or anybody who could take the delegator's role back sets `revoked_at` and signs the delegate out everywhere, while anybody else is answered 404 so the endpoint reveals neither the delegation nor its state, and revoking the underlying role takes back what it had lent in the same transaction. Both write `DELEGATION_CREATED`/`DELEGATION_REVOKED` to `audit_logs` and email the delegate (and the delegator when somebody else ended it) | US-002 asks for temporary authority within a date range; keeping it outside `user_roles` keeps the role history honest and makes the "delegated by" stamp of Epic 4 traceable |
| Denied requests | Not specified | Every refusal of a signed-in caller — request rules and method security — is published as an authorization event and written to `audit_logs` as `ACCESS_DENIED` (entity type `AUTHORIZATION`, own transaction, failure logged rather than surfaced) with method, path, the missing permission, authority or scope, IP and correlation id; the 403 body is `urn:awards:problem:access-denied` with the same detail. A directory lookup outside the caller's scope answers 404 so foreign ids are not confirmed | One listener covers endpoints added later; a typed body lets the browser app explain the refusal |

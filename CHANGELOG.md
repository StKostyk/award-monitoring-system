...
### Added
- Approval authority delegation: `GET`/`POST /api/v1/delegations` and `DELETE /api/v1/delegations/{id}` lend an approval role to a colleague for at most 90 days (`role_delegations`, V018; typed refusals `delegation-not-holder`, `delegation-bad-delegate`, `delegation-period`, `delegation-overlap`, `delegation-not-active`). The delegate's token gains the `delegations` claim, the borrowed `role_scopes` entry and the role's reading and approving permissions only — never user management, and a borrowed role cannot be lent on. A delegation expires by calendar date without any job; revoking it, or revoking the underlying role, signs the delegate out everywhere. Both are audited (`DELEGATION_CREATED`, `DELEGATION_REVOKED`) and emailed; Angular `/delegations` page with state chips, delegate dialog and the "acting for" banner in the header
- Role assignment and membership confirmation: `POST /api/v1/users/{id}/roles` and `DELETE /api/v1/users/{id}/roles/{roleId}` grant and take back roles below the caller's level inside their organisation subtree (typed refusals `role-above-level`, `role-organization-mismatch`, `role-already-assigned`, `role-last-own`); revocation ends the assignment yesterday and signs the holder out everywhere; both are audited (`ROLE_ASSIGNED`, `ROLE_REVOKED`) and emailed; `uk_user_roles_current` keeps a role unique per organisation (V017); Angular `/admin/users` and `/admin/users/:id` with filters, the unconfirmed badge, one-click confirmation and assign/revoke dialogs
- Organisation-scoped permission model: access tokens carry `role_scopes`; `@PreAuthorize` rules resolve "inside my subtree" and "below my level" from an in-memory organisation tree; every refusal is audited (`ACCESS_DENIED`) and answered with a typed 403 naming the missing permission or scope; `user:read:scope` and `user:manage:scope` for the approval roles; user directory `GET /api/v1/users` (scope, organisation, role, status, unconfirmed, free text; pages of up to 100) and `GET /api/v1/users/{id}` with the role history
...
### Fixed
- Sign-in flow after the manual run: the browser app returns to the login page when its refresh token stops working instead of showing an empty page, and starts even when its stored token has expired; a login form posted with a stale CSRF token (after a server restart) explains that the page expired; signing in at `/login` directly, `/login` while signed in and `/` all land on the app; the authorization server has a branded error page; a password reset refuses the current password (`password-same-as-current`); a reset or "this was not me" now refuses access tokens issued before it at once (`auth:nbf:` in Redis, fail-open); the dev start script warns when `localhost:6379` is not the compose Redis; a 429 from the request limit now carries the CORS headers so the browser app can tell it from an outage, and the `local` profile allows 120 requests a minute; login sessions are tracked by cookie only (no `;jsessionid=` in URLs)
